            ...
            switch (s_currentTab)
            {
                ...
                case Tabs.Materials:
                    if (m_result.Data != null && m_result.VrmExtension != null)
                    {
                        ...
                        var materialTypes = System.Enum.GetValues(typeof(ImportMaterialTypes)).Cast<ImportMaterialTypes>().Select(x => x.ToString()).ToArray();
                        var materialTypeProperty = serializedObject.FindProperty(nameof(VrmScriptedImporter.MaterialType));
                        materialTypeProperty.enumValueIndex = EditorGUILayout.Popup("Material Type", materialTypeProperty.enumValueIndex, materialTypes);
